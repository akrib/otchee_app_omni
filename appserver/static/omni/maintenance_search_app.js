/*
 *  Omni Maintenance Search App  -  appli mono-page (full JS) pour ITSI Maintenance
 *  Remplace : itsi__maintenance_search.xml (gros SPL data2HTML)
 *             + convert_token_to_html.js + tooltip.js  ->  1 seul JS
 *
 *  Principe (aligne sur maintenance_app.js) :
 *   - 1 seule vue XML (cf. itsi__maintenance_search.xml) avec #omni_maintenance_search_app
 *   - UI.shell() construit header + toolbar + zone resultats + pagination + loader + modal
 *   - SearchHub agrege le doneProgress => loader global en %
 *   - Le SPL renvoie des DONNEES STRUCTUREES (plus de data2HTML cote SPL).
 *     => le HTML des cartes est construit ici, en JS.
 *   - Recherche INSTANTANEE cote client : on charge une fois (a la date choisie),
 *     puis filtrage / tri / pagination en memoire. Re-recherche serveur uniquement
 *     quand la date change (car flag_dt = "actif a cette date" est calcule par
 *     la commande omnidowntimecalculation).
 */
...
console.log('%c %s', 'background:#222;color:#bada55',
  'Omni Maintenance Search App v' + APP_VERSION + ' charge');
...
require([
   ...
   * ============================================================ */
  var SPL = {
    // RBAC : filtre client en fonction des roles (repris de l'ancien base_filtre)
    clientFilter: ''
    ...
    // Donnees structurees (PLUS de data2HTML). clientWhere : "| search (client=...)" ou ""
    main: function (epoch, clientWhere) {
      return ''
  ...
  // ordre des colonnes du | table ci-dessus -> mapping ligne -> objet
  ...
  /* ============================================================
   *  STORE  -  etat de recherche
   * ============================================================ */
  var Store = {
    all: [], filtered: [],
    page: 0, term: '', activeOnly: false, category: 'all', sort: 'update',
    epoch: Util.epochFromInput(null), loadTime: 0, clientWhere: '',
    hlTerms: [],
    lastQueries: {}   // id -> SPL (pour le panneau debug)
  };
  ...
  var UI = {
    css: function () {
      if ($('#omni-search-style').length) return;
      var c = [
        ...
        '#omni_maintenance_search_app{font-family:Poppins,system-ui,Segoe UI,Roboto,sans-serif;color:var(--omni-ink);max-width:1180px;margin:0 auto;}',
        '#omni_maintenance_search_app *{box-sizing:border-box;}',
      ...
      $('<style id="omni-search-style">').text(c).appendTo('head');
    },
    ...
    shell: function () {
      var html = ''
        ...
        + '    <h1>Maintenance Search</h1>'
        + '    <span class="omni-badge">search</span>'
        + '  </div>'
        /* toolbar */
        ...
        + '               placeholder="Hote, site, sonde, auteur, commentaire, ticket… (plusieurs mots = ET)"/>'
        ...
        + '</div></div>';
      $('#omni_maintenance_search_app').html(html);
  ...
  var Render = {
    ...
    card: function (m, terms) {
      ...
      // >>> Liens vers la nouvelle vue unifiee (itsi__maintenance.xml ?mode=...).
      //     (Decommentez les anciens liens si vous gardez les vues historiques.)
  ...
  /* ============================================================
   *  APP  -  recherche / filtrage / tri / pagination
   * ============================================================ */
  var SearchApp = {

    init: function () {
      log('init : ' + Config.view + ' | clientScope=' + Config.clientScope
        + ' | rows/page=' + Config.rowsPerPage, 'etape > init');
      SearchApp.bindToolbar();
      SearchApp.loadData();
    },

    /* --- chargement serveur (a la date choisie) --- */
    loadData: function () {
      log('chargement serveur | epoch=' + Store.epoch
        + ' (' + new Date(Store.epoch * 1000).toLocaleString() + ')', 'etape > loadData');
      var t0 = Date.now();
      var run = function () {
        SearchHub.run('main', SPL.main(Store.epoch, Store.clientWhere), {
          message: 'Chargement des maintenances…',
          count: 0,
          earliest: '0', latest: 'now',   // all-time : inputlookup + omnidowntimecalculation
          onResults: function (rows) {
            Store.all = rows.map(rowToObj);
            Store.loadTime = ((Date.now() - t0) / 1000).toFixed(2);
            log('maintenances chargees : ' + Store.all.length + ' en ' + Store.loadTime + 's',
              'etape > donnees recues');
            SearchApp.applyFilters();
          },
          onError: function () {
      ...
      // RBAC optionnel : on resout d'abord le filtre client
      if (Config.clientScope) {
        SearchHub.run('clientFilter', SPL.clientFilter, {
          ...
          onResults: function (rows) {
            ...
            // admin (client=*) ou aucun filtre -> on ne restreint pas
    ...
    /* --- filtrage + tri (cote client, instantane) --- */
    ...
    render: function () {
      ...
      if (!slice.length) {
        $('#omni-results').html('<div class="omni-empty">'
          + (Store.all.length ? 'Aucune maintenance ne correspond a vos criteres.' : 'Aucune maintenance enregistree.')
          + '</div>');
      } else {
        $('#omni-results').html(slice.map(function (m) { return Render.card(m, terms); }).join(''));
      }

      SearchApp.renderPagination();
      log('page ' + (Store.page + 1) + ' | ' + slice.length + ' carte(s) affichee(s)', 'etape > render');
    },
    ...
    bindToolbar: function () {
      ...
      $('#omni-cat .omni-chip').on('click', function () {
        ...
        log('type = ' + Store.category, 'etape > filtre type');
        SearchApp.applyFilters();
      });

      $('#omni-sort').on('change', function () {
        Store.sort = this.value;
        log('tri = ' + Store.sort, 'etape > tri');
        SearchApp.applyFilters();
      });

      $('#omni-active-only').on('change', function () {
        Store.activeOnly = this.checked;
        log('en cours uniquement = ' + Store.activeOnly, 'etape > filtre en cours');
        SearchApp.applyFilters();
      });

      // changement de date -> recalcul serveur (flag_dt depend de la date)
      ...
      // panneau Debug (present uniquement si mode debug actif)
      $('#omni-debug').on('click', SearchApp.showDebug);
    },

    /* --- panneau Debug : requetes SPL utilisees + etat courant --- */
    showDebug: function () {
      ...
      var etat = {
        view: Config.view, clientScope: Config.clientScope, rowsPerPage: Config.rowsPerPage,
