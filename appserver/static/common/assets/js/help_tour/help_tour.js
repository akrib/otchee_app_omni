...
require([
  "jquery",
  "splunkjs/mvc",
  'css!../app/' + APP_NAME + '/common/assets/js/help_tour/lib/driver.css',
  'css!../app/' + APP_NAME + '/common/assets/js/help_tour/lib/semantic.min.css',
...
function driver(e = {}) {
  ...
  // Variable pour stocker l'instance du driver
  var driverObj = null;

  $(document).ready(function () {

    // Fonction pour générer les steps visibles
    function getVisibleSteps() {
      var formated_steps = [];
      
      $("[id^=help_tour_]").each(function (i, el) {
        // Vérifier si l'élément cible est visible
        var targetElement = $(evalHtmlString($(el).attr('data-element')));
    ...
    // Créer le bouton d'aide une seule fois
    if ($("#HELP_TOUR_BUTTON").length > 0) {
      console.log("HELP_TOUR_BUTTON already exist");
    } else {
      $('[data-view="views/shared/appbar/AppLabel"]').before(
        "<a class='btn btn-link' id='HELP_TOUR_BUTTON'><i class='question circle big icon'></i></a>"
      );
    }

    // Event listener sur le bouton d'aide
    $(document).on('click', '#HELP_TOUR_BUTTON', function () {
      console.log("Lancement du tour d'aide");
      ...
      // Récupérer les steps visibles au moment du clic
      var visibleSteps = getVisibleSteps();
      
      if (visibleSteps.length === 0) {
        console.warn("Aucun élément d'aide visible trouvé");
        alert("Aucun élément d'aide n'est actuellement visible sur cette page.");
        return;
      }
      
      console.log("Steps visibles trouvés:", visibleSteps.length);
      
      // Créer une nouvelle instance avec les steps visibles
      driverObj = driver({
        showProgress: true,
        steps: visibleSteps
      });
  ...
  function genStepWithData(count, step) {
    return {
      element: evalHtmlString($(step).attr('data-element')) || ".dashboard-header-title",
      popover: {
        title: evalHtmlString($(step).attr('data-title')) || "titre",
        description: evalHtmlString($(step).attr('data-description')) || "description",
        side: evalHtmlString($(step).attr('data-side')) || "left",
        align: evalHtmlString($(step).attr('data-align')) || "start"
      } 
    };
  }
